...
function activateCountry(d){
  ...
  detailTemplate = Hogan.compile("<div class='wrapper'><div id='btn-close'>×</div><h1 class='no-caps-title pb1'>{{name__localised}}</h1><div class='status-block'><h2 class='mv2'>{{status__localised}}</h2></div>{{#since}}<div class='since-date'><h3 class='since-header'>" + dictionary.getTranslation('SINCE') + " {{since}}</h3></div>{{/since}}<div class='definition'><p class='mv2 detailBox-definition'>{{definition__localised}}</p></div></div><div class='totals-block'><div class='media bg-white'>{{#death-penalties}}<div class='media__img dp-image'><img class='death-sentences-icon' src='images/hammer.svg'></div><div class='media__body'><h2 class='dp-header' id='dp-header'>{{death-penalties}}</h2></div><h3 class='dp-words lh-reset'>" + dictionary.getTranslation('DEATH SENTENCES') + "</h3>{{/death-penalties}}</div><div class='media bg-black white'>{{#executions}}<div class='media__img executions-image'><img class='executions-icon' src='images/WhiteNoose.svg'></div><div class='media__body'><h2 class='execution-header' id='execution-header'>{{executions}}</h2></div><h3 class='execution-words lh-reset'>" + dictionary.getTranslation('EXECUTIONS') + "</h3>{{/executions}}</div></div></div>");
